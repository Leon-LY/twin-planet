...
> **颜色按出生顺序分配，不按性别。** 大宝=蓝 `#4299E1`，二宝=粉 `#F56565`，
> 适用于两个男孩/两个女孩/龙凤胎任何组合。颜色是身份标识，不是性别标签。

| 用途 | Hex | 适用 |
|------|------|------|
| 出生顺序 1 (大宝) | `#4299E1` | 任何性别 |
| 出生顺序 2 (二宝) | `#F56565` | 任何性别 |
...
> **颜色按出生顺序，不按性别。** 大宝=蓝 `#4299E1`，二宝=粉 `#F56565`。
> 无论是两个男孩、两个女孩、还是一男一女，规则不变。
> 颜色是身份标识（一眼认出是谁的数据），性别通过 emoji/头像/名字区分。
...
| 用途 | 色值 | 场景 |
|------|------|------|
| 出生顺序1(大宝) | `#4299E1` | 左侧卡片、身份标记、实测曲线（不按性别） |
| 出生顺序2(二宝) | `#F56565` | 右侧卡片、身份标记、停止按钮（不按性别） |
| 背景 | `#FFFBF5` | 全局页面底色 |
| 卡片 | `#FFFFFF` | 内容区底色 |
| 主文字 | `#2D3748` | 标题、正文 |
| 次文字 | `#A0AEC0` | 描述、时间戳 |
