    ...
    <template v-if="!recordsStore.isRunning">
      ...
      <view class="baby-tabs">
        <view class="baby-tab" v-for="(t,i) in twins" :key="t.id"
          :class="{ active: sel===t.id, 'tab-a':i===0, 'tab-b':i===1 }"
          @click="sel=t.id">
      ...
      <!-- 盖章网格 — 3×2 but with character -->
      <view class="stamp-grid">
        <view v-for="(a,i) in actions" :key="a.type"
          class="stamp-btn"
          :class="{
            'stamp-primary': i===0,
            'stamp-secondary': i===1,
      ...
      <!-- 回溯 — margin-note style -->
      <view class="retro-note">
        <text class="retro-dash">--</text>
        <text class="retro-label">刚才忘了？</text>
        <view class="retro-type-row">
          <text v-for="a in retroActions" :key="a.type"
            class="retro-type-dot" :class="{ on: retroType===a.type }"
            @click="retroType=a.type">{{ a.emoji }}</text>
        </view>
...
function popSticker(emoji:string){
  if(stickerTimer)clearTimeout(stickerTimer)
  stickerEmoji.value=emoji;stickerShow.value=true
...
function syncStickers(){
  ...
  const aId=babiesStore.babyA?.id;const bId=babiesStore.babyB?.id;const n=Date.now()
  stickersStore.sync({
    todayLogCount:today.length,streakDays:recordsStore.streakDays,
    totalLogCount:recordsStore.logs.length,
    twinSyncCount:today.filter(l=>l.type==='feeding'||l.type==='sleep').length>=2?1:0,
    ...
    babyARecentRecord:aId?today.some(l=>l.babyId===aId&&n-l.createdAt<3600000):false,
    babyBRecentRecord:bId?today.some(l=>l.babyId===bId&&n-l.createdAt<3600000):false,
  })
}
...
function doAction(t:RecordType){
  ...
  if(t==='feeding'||t==='sleep'){recordsStore.startTimer(id,t);haptic.thump();popSticker(t==='feeding'?'🍼':'😴')}
  else{recordsStore.quickLog(id,t);haptic.sparkle();syncStickers()
...
function dualLog(t:RecordType){
  ...
  if(a)recordsStore.quickLog(a.id,t);if(b)recordsStore.quickLog(b.id,t)
  haptic.doubleBeat();syncStickers();popSticker('🔗')
}
const retroType=ref<RecordType>('feeding')
function retro(m:number){
  const id=sel.value||twins.value[0]?.id;if(!id)return
  recordsStore.quickLog(id,retroType.value,undefined,m*60000);haptic.sparkle();syncStickers();popSticker('⏰')
}
function stopOne(id:string){
  const l=recordsStore.stopTimer(id);haptic.thump();syncStickers()
  if(l){popSticker(l.type==='feeding'?'🍼':l.type==='sleep'?'😴':'✅')}
}
...
.baby-tab.active{opacity:1}
.baby-tab.tab-a.active{border-bottom-color:var(--amber);background:linear-gradient(to top,var(--amber-lt),transparent 60%)}
.baby-tab.tab-b.active{border-bottom-color:var(--rose);background:linear-gradient(to top,var(--rose-lt),transparent 60%)}
.tab-emoji{font-size:40rpx;transition:transform .3s var(--ease-bounce)}
.baby-tab:active .tab-emoji{transform:scale(1.2)}
.tab-name{font-family:var(--font-journal);font-size:26rpx;font-weight:700;color:var(--ink)}
.tab-check{font-size:20rpx;color:var(--mint)}
...
.stamp-btn:active{transform:scale(.9);border-color:var(--amber);background:var(--amber-lt)}
.stamp-btn.stamp-primary{padding:32rpx 8rpx}
.stamp-btn.stamp-secondary{padding:30rpx 8rpx}
.stamp-btn.stamp-small{padding:22rpx 8rpx}
.stamp-emoji{font-size:36rpx;transition:transform .3s var(--ease-bounce)}
.stamp-primary .stamp-emoji{font-size:44rpx}
.stamp-secondary .stamp-emoji{font-size:40rpx}
.stamp-small .stamp-emoji{font-size:30rpx}
.stamp-btn:active .stamp-emoji{transform:scale(1.3)}
.stamp-label{font-size:24rpx;font-weight:600;color:var(--ink)}
.stamp-primary .stamp-label{font-size:28rpx}
.stamp-small .stamp-label{font-size:20rpx;color:var(--ink-md)}

/* 双宝同步 */
.dual-row{display:flex;gap:10rpx;margin-bottom:20rpx;position:relative;z-index:1;align-items:center}
.dual-chip{flex:1;text-align:center;padding:14rpx 8rpx;background:var(--cream);border:1.5px solid var(--dot);border-radius:24rpx;font-size:22rpx;color:var(--ink-md);font-weight:600;transition:transform .15s var(--ease-bounce)}
...
.dual-chip:not(.dual-primary){font-size:20rpx}

/* 回溯 — margin note */
.retro-note{display:flex;align-items:center;gap:10rpx;flex-wrap:wrap;justify-content:center;position:relative;z-index:1;padding:8rpx 0}
.retro-dash{color:var(--ink-lt);font-size:20rpx;opacity:.5}
.retro-label{font-size:22rpx;color:var(--ink-lt)}
.retro-type-row{display:flex;gap:6rpx}
.retro-type-dot{width:44rpx;height:44rpx;display:flex;align-items:center;justify-content:center;font-size:24rpx;border-radius:50%;background:var(--cream);border:1.5px solid var(--dot);transition:border-color .2s,background .2s,transform .15s var(--ease-bounce)}
.retro-type-dot:active{transform:scale(.85)}
.retro-type-dot.on{border-color:var(--amber);background:var(--amber-lt)}
.retro-times{display:flex;gap:6rpx}
.retro-min{padding:6rpx 14rpx;font-size:20rpx;color:var(--ink-lt);background:var(--cream);border:1px solid var(--dot);border-radius:16rpx;transition:transform .15s var(--ease-bounce)}
.retro-min:active{transform:scale(.9);border-color:var(--amber)}

/* 计时器 hero */
.timer-hero{display:flex;flex-direction:column;align-items:center;padding:48rpx 0;position:relative;z-index:1}
.hero-face{width:220rpx;height:220rpx;border-radius:50%;display:flex;align-items:center;justify-content:center;margin-bottom:20rpx}
.hero-face.bg-a{background:var(--amber-md)}.hero-face.bg-b{background:var(--rose-md)}
.hero-emoji{font-size:108rpx;animation:faceRock .8s ease-in-out infinite}
@keyframes faceRock{0%,100%{transform:rotate(0)}30%{transform:rotate(-5deg)}70%{transform:rotate(5deg)}}
.hero-baby-name{font-family:var(--font-journal);font-size:40rpx;color:var(--ink)}
.hero-phase{font-size:28rpx;color:var(--mint);margin-top:4rpx;font-weight:600}
.hero-clock{display:flex;align-items:baseline;margin-top:8rpx;gap:0}
.clock-min{font-family:var(--font-journal);font-size:72rpx;color:var(--ink);letter-spacing:4rpx}
.clock-sep{font-size:40rpx;color:var(--ink-lt);margin:0 4rpx;animation:sepBlink 1s step-end infinite}
@keyframes sepBlink{0%,100%{opacity:1}50%{opacity:.2}}
.clock-sec{font-family:var(--font-journal);font-size:48rpx;color:var(--ink-md)}
.end-btn{margin-top:28rpx;padding:18rpx 48rpx;background:transparent;border:2rpx solid var(--twin-danger);border-radius:28rpx;color:var(--twin-danger);font-size:28rpx;font-weight:600;transition:transform .15s var(--ease-bounce)}
.end-btn:active{transform:scale(.94);background:rgba(212,112,107,.06)}

/* 双计时 */
.dual-zone{display:flex;gap:12rpx;margin-bottom:12rpx;position:relative;z-index:1}
.dual-card{flex:1;display:flex;flex-direction:column;align-items:center;gap:10rpx;padding:20rpx 8rpx;border-radius:24rpx}
...
.dc-emoji{font-size:48rpx;animation:faceRock .8s ease-in-out infinite}
...
.dc-stop{padding:6rpx 20rpx;border-radius:16rpx;border:1.5px solid var(--twin-danger);font-size:20rpx;color:var(--twin-danger)}
.dc-stop:active{background:rgba(212,112,107,.06)}
.stop-all{width:100%;padding:16rpx;background:transparent;border:2rpx solid var(--twin-danger);border-radius:24rpx;font-size:28rpx;font-weight:600;color:var(--twin-danger);position:relative;z-index:1}
