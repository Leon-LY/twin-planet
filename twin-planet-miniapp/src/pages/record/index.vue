    ...
    <template v-if="recordsStore.runningTimers.length === 1">
      <view class="timer-hero">
        <view class="hero-face" :class="runningTwin==='a'?'face-a':'face-b'">
          <text class="face-big">{{ runningFace }}</text>
        </view>
      ...
      <view class="dual-zone">
        <view class="dual-card" v-for="t in recordsStore.runningTimers" :key="t.babyId">
          <view class="dual-face" :class="t.babyId===twins[0]?.id?'face-a':'face-b'">
            <text class="face-big">{{ t.babyId===twins[0]?.id ? '😋' : '😴' }}</text>
          </view>
...
const runningFace=computed(()=>recordsStore.runningTimer?.type==='sleep'?'😴':'😋')
const poeticLabel=computed(()=>{
  const p=recordsStore.runningTimer?.type==='sleep'?[{m:2,l:'刚刚入眠'},{m:5,l:'星尘落下'},{m:10,l:'梦乡深处'},{m:20,l:'星际漫游'},{m:999,l:'继续守护'}]:[{m:2,l:'刚刚开始'},{m:5,l:'星光降临'},{m:10,l:'航行中'},{m:15,l:'渐入佳境'},{m:20,l:'即将圆满'},{m:999,l:'星光已满'}]
  const min=runningElapsed.value/60;for(const x of p){if(min<x.m)return x.l};return p[p.length-1].l
})
...
.qd-btn { flex:1; text-align:center; padding:18rpx 8rpx; background:var(--amber-bg); border:1.5px solid var(--amber); border-radius:var(--radius-full); font-size:var(--font-caption); color:var(--amber); font-weight:700; }
.qd-btn:active { transform:scale(0.94); background:var(--amber); color:#FFF; }

.retro { text-align:center; margin-bottom:24rpx; }
.retro-chips { display:flex; justify-content:center; gap:12rpx; margin-top:8rpx; }
.r-chip { padding:10rpx 20rpx; background:var(--paper-warm); border:1px solid var(--dot-line); border-radius:var(--radius-full); font-size:var(--font-caption); color:var(--ink-md); }
...
.stop-btn { margin-top:32rpx; padding:22rpx 56rpx; background:transparent; border:2rpx solid var(--twin-danger); border-radius:var(--radius-full); color:var(--twin-danger); font-size:var(--font-body); font-weight:600; }
.stop-btn:active { background:rgba(212,112,107,0.06); transform:scale(0.95); }
